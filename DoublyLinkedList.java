public class DoublyCircularLinkedList {
    ...
    public static void main(String[] args) {
        ...
        list.displayForward();   // 10 <-> 15 <-> 20 <-> (back to head)
        list.displayBackward();  // 20 <-> 15 <-> 10 <-> (back to tail)

        list.deleteAtHead();
        list.displayForward();   // 15 <-> 20 <-> (back to head)

        list.deleteAtTail();
        list.displayForward();   // 15 <-> (back to head)

        list.insertAtTail(30);
        list.insertAtTail(40);
        list.insertAtPosition(1, 25);
        list.displayForward();   // 15 <-> 25 <-> 30 <-> 40 <-> (back to head)

        int pos = list.search(30);
        System.out.println("Position of 30: " + (pos != -1 ? pos : "Not found"));

        list.updateAtPosition(1, 100); // 25 -> 100
        list.updateAtPosition(3, 99);  // 40 -> 99
        list.displayForward();         // 15 <-> 100 <-> 30 <-> 99 <-> (back to head)
